...
class SVG {
  ...
  addElem(e, content, args={}) {
    const element = document.createElementNS(__ns, e)
    ...
    if (args.invisible) {
      ...
      this.addToLayer(element, 'blah', 'none')
    }
  ...
  drawPath(x, y, d, args={}) {
    ...
    if (args.turbulance) {
      x += rnd(-1*args.turbulance, args.turbulance)
      y += rnd(-1*args.turbulance, args.turbulance)
    }
    if (!isLetter) {
      x += this.chaosFn()
      y += this.chaosFn()
    ...
    path.setAttribute('style', `transform: translate(${x}px, ${y}px) scale(${size}) rotate(${rotation}deg); ${style}`)
