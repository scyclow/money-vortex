...
function generateCSVRow(attrs, tokenData) {
  return [
    mantras[tokenData.tokenId],
    attrs.emoji,
    capitalize(attrs.bw),
    capitalize(attrs.rosetteType),
    capitalize(attrs.colorStrategy),
    attrs.hasAura ? 'true' : 'false',
    attrs.hasPump ? 'true' : 'false',
    attrs.rotation > 0 ? 'true' : 'false',
  ].join(',')
}


const csv = [
  ['Mantra', 'Icon', 'Brain Wave', 'Rosette Type', 'Spectrum', 'Aura', 'Pump', 'Rotation'].join(',')
]
...
for (let i = 0; i < hashes.length; i++) {
  ...
  csv.push(generateCSVRow(attrs, tokenData))

  writeFileSync(`./docs/metadata/attributes.csv`, csv.join('\n'));
}
